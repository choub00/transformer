...
# Test on actual paragraph from the document
...
for elem in body.iter(w('p')):
    text = get_all_text(elem).strip()
    if len(text) > 20 and has_non_black_run(elem):
        count += 1
        if count <= 3:
            print('Paragraph ' + str(count) + ':')
            print('Original (' + str(len(text)) + ' chars):')
            print(text)
            print('')
            
            new_text = academic_rewrite(text)
            print('Rewritten (' + str(len(new_text)) + ' chars):')
            print(new_text)
            print('')
