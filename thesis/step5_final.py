...
# ============================================================
# EXACT TEXT FROM DOCX - extracted and hardcoded
# ============================================================
...
def rw_p106(text):
    # 原文有两处冒号，全句替换
    old = (
        '在真实投资决策中，模型至少需要满足三项要求：能够在横截面上识别资产相对强弱，'
        '能够在不同市场状态下保持基本稳健，能够在考虑交易成本后仍保留可解释的净收益'
    )
    new = (
        '在真实投资决策中，模型至少要满足三项条件，一是在横截面上能分出哪些资产相对更好，'
        '二是行情切换时基本保持有效不至于完全失效，三是扣掉交易摩擦后净收益仍具有可解释性'
    )
...
def rw_p120(text):
    # 原文：四个方面：第一点，...；第二点，...；第三点，...；第四点，...
    old = (
        '围绕上述目标，研究内容分为四个方面：第一点，构建适用于多资产股票预测任务的模型框架，'
        '比较不同版本模型在金融非平稳环境中的表现差异；第二点，'
        '设计防泄露训练与回测评估机制，用滚动窗口标准化、标签时序对齐和 Walk-Forward 验证提升实验结论可信度；'
        '第三点，在回测阶段引入交易成本建模和 Anti-Churn 调仓机制，检验模型输出在真实执行条件下是否可用；'
        '第四点，基于 FastAPI 与 Vue 3 实现前后端一体化系统原型，使模型训练、预测、回测和可视化分析形成闭环'
    )
    new = (
        '围绕前述目标，本文主要开展四个方面的工作，一是在多资产股票预测场景下构建模型框架，'
        '对比 v1、v2、v3 在金融非平稳条件下的性能差异，二是在防泄露训练与回测评估体系上，'
        '通过滚动窗口标准化、标签时序对齐和 Walk-Forward 验证来提升实验结论可信度，'
        '三是在回测环节纳入交易成本建模和 Anti-Churn 调仓机制，考察模型输出在考虑真实摩擦成本后是否仍具可用性，'
        '四是以 FastAPI 和 Vue 3 为基础实现前后端一体的系统原型，将模型训练、预测、回测和可视化分析串联为完整链路'
    )
...
def rw_p125(text):
    result = text
    old1 = (
        '研究方法上，本课题采用文献研究法、系统设计法、实验分析法和案例分析法。'
        '文献研究法用于梳理股票预测、Transformer 调整、状态空间模型和量化交易评估等方向推进脉络，'
        '系统设计法用于搭建数据层、模型层、服务层和展示层的一体化框架，'
        '实验分析法用于比较 v1、v2、v3 在 IC、Sharpe Ratio、最大回撤和换手率等指标上的差异，'
        '案例分析法聚焦第 6 章的 2024 年震荡市样本，用来观察模型在复杂市场环境中的表现'
    )
    new1 = (
        '在研究方法上，本文综合使用文献研究、系统设计、实验分析和案例研究四种方式。'
        '文献研究用来梳理股票预测、Transformer 改进、状态空间模型和量化评估等方面的发展脉络，'
        '系统设计用于搭建数据层、模型层、服务层和展示层的完整框架，'
        '实验分析通过 IC、Sharpe Ratio、最大回撤和换手率等指标对比 v1、v2、v3 的实际表现，'
        '案例研究则聚焦第六章中的 2024 年震荡市场样本，考察模型在复杂环境下的行为特征'
    )
...
def rw_p154(text):
    ...
    old2 = (
        '数据层负责历史行情接入、股票池维护、特征工程、标签构造和滚动标准化，是系统输入来源，'
        '模型层以 AlphaTransformer 系列模型为关键，负责训练、验证、推理和版本迭代，'
        '服务层基于 FastAPI 建立，封装预测接口、回测接口、健康检查和模型状态管理，'
        '用来隔离底层模型细节并一致外部调用协议，展示层基于 Vue 3 实现，'
        '将模型输出转化为图表、表格和交互面板。该样的分层方式，让算法研究和系统落地之间形成较清晰的结构映射'
    )
    new2 = (
        '数据层负责历史行情接入、股票池维护、特征工程、标签构造和滚动标准化，为整个系统提供数据输入，'
        '模型层以 AlphaTransformer 系列模型为核心，承担训练、验证、推理和版本迭代等任务，'
        '服务层基于 FastAPI 构建，封装了预测接口、回测接口、健康检查和模型状态管理等功能，'
        '隔离底层细节并统一对外调用协议，展示层采用 Vue 3 实现，'
        '将模型输出转化为图表、表格和交互面板供用户查看。'
        '上述分层方式在算法研究与系统落地之间建立了较为清晰的对应关系'
    )
    return result.replace(old2, new2)
...
def rw_p174(text):
    ...
    old2 = (
        '论文先用自编码器重建输入特征，以重建误差衡量当前样本偏离常态分布的程度，'
        '再把该误差与时序池化特征拼接，生成状态概率向量。状态概率不直接决定最终交易，'
        '主要是进入自适应归一化层，控制不同状态下的缩放和偏移'
    )
    new2 = (
        '具体实现上，先用自编码器重建输入特征，以重建误差来量化当前样本偏离常态分布的程度，'
        '再将该误差与时序池化特征拼接得到状态概率向量。状态概率不直接决定交易动作，'
        '而是送入自适应归一化层，对不同状态下的缩放和偏移参数进行动态调节'
    )
    result = result.replace(old2, new2)
    old3 = (
        '以 2024 年震荡市为例，指数整体波动有限，行业轮动速度较快，热点持续时间较短，'
        '当时若模型仍按平稳阶段尺度理解成交量异常和短线反弹，就容易高估局部噪声的持续性。'
        'Regime-Aware 的作用，正是识别相关环境变化，并调整模型对同一特征响应强度'
    )
    new3 = (
        '以 2024 年震荡市为例，指数整体振幅有限，行业轮动加快，热点持续时间缩短。'
        '在这段时间里，如果模型仍然按照平稳阶段的特征尺度来解读成交量异常和短线反弹，'
        '就容易将局部噪声误认为持续性信号。'
        'Regime-Aware 的核心功能就在于识别这类环境变化，并相应调整模型对同一特征的反应强度'
    )
    return result.replace(old3, new3)
...
def rw_p220(text):
    ...
    old2 = (
        '主实验用于比较 v1、v2、v3 的整体性能跃迁，'
        '消融实验用于验证 Regime-Aware、Mamba-2、iTransformer 与交易约束项各自的边际贡献，'
        '鲁棒性实验则聚焦高波动、急跌和风格切换阶段，验证模型在分布突变条件下是否仍保持平稳'
    )
    new2 = (
        '主实验用来对比 v1、v2、v3 三代模型的整体性能差异，'
        '消融实验用来考察 Regime-Aware、Mamba-2、iTransformer 以及交易约束项各自对最终效果的边际贡献，'
    ...
    new3 = (
        '这三类实验组合在一起，可以分别回答三个层次的问题。'
        '模型相比基线是否有提升、提升究竟来自哪些模块、以及这些提升是否只在特定样本区间内偶然出现'
    )
    return result.replace(old3, new3)
...
def rw_p270(text):
    ...
    old3 = (
        '指导教师在论文选题、研究思路、系统实现和论文修改等方面给予耐心指导，'
        '使我逐步完成从算法设计到系统落地的全过程；'
        '学院提供的学习环境与实验条件，为课题顺利开展提供了保障；'
        '家人的理解与支持，也是我完成本次毕业设计的重要动力'
    )
    new3 = (
        '指导教师在选题方向、研究思路、系统实现和论文修改等环节给予耐心指点，'
        '帮助我逐步完成从算法设计到系统落地的完整过程，'
        '学院提供的学习环境和实验条件为课题的顺利推进创造了条件，'
        '家人的理解与支持则是完成此次毕业设计的重要动力'
    )
